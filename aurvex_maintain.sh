...
PYEOF
    ...
    if [ "${UPDATED:-false}" = "true" ] || [ "$ERRORS" -gt 0 ]; then
        ...
        sleep 8
        systemctl is-active --quiet aurvex-bot && fixed "Bot yeniden başlatıldı" || err "Bot başlatılamadı"
    fi
else
...
systemctl is-active aurvex-bot      2>/dev/null && echo -e "  aurvex-bot:       ${GREEN}✅ çalışıyor${NC}" || echo -e "  aurvex-bot:       ${RED}❌ durmuş${NC}"
systemctl is-active aurvex-dashboard 2>/dev/null && echo -e "  aurvex-dashboard: ${GREEN}✅ çalışıyor${NC}" || echo -e "  aurvex-dashboard: ${RED}❌ durmuş${NC}"
