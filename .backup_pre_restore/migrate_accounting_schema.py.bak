"""
scripts/migrate_accounting_schema.py — DB Migration v5.1
========================================================
Mevcut DB'ye eksik kolonları ve tabloları ekler.
Eski veriyi ASLA silmez. _TRADE_COLUMNS cache'ini sıfırlar.
"""
import os
import sys
import sqlite3

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from config import DB_PATH
from database import init_db


def migrate():
    print(f"[Migration] DB: {DB_PATH}")

    init_db()

    trade_columns = {
        "qty_tp1":            "REAL DEFAULT 0",
        "qty_tp2":            "REAL DEFAULT 0",
        "qty_runner":         "REAL DEFAULT 0",
        "notional_size":      "REAL DEFAULT 0",
        "margin_used":        "REAL DEFAULT 0",
        "risk_pct":           "REAL DEFAULT 1.0",
        "risk_usd":           "REAL DEFAULT 0",
        "max_loss_after_fee": "REAL DEFAULT 0",
        "unrealized_pnl":     "REAL DEFAULT 0",
        "open_fee":           "REAL DEFAULT 0",
        "close_fee":          "REAL DEFAULT 0",
        "fee_rate":           "REAL DEFAULT 0.0004",
        "duration_seconds":   "INTEGER DEFAULT 0",
        "market_regime":      "TEXT",
        "is_valid_for_stats": "INTEGER DEFAULT 1",
        "archived_reason":    "TEXT",
        "tp3":                "REAL",
        "remaining_qty":      "REAL",
        "original_qty":       "REAL",
        # v5.1 — eksik kolonlar eklendi
        "close_price":        "REAL",
        "mark_price":         "REAL",
        "last_update":        "TEXT",
        "source":             "TEXT DEFAULT 'bot'",
    }

    coin_profile_columns = {
        "short_bias": "REAL DEFAULT 0.5",
    }

    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    # trades tablosu
    cursor.execute("PRAGMA table_info(trades)")
    existing_trades = {row[1] for row in cursor.fetchall()}

    added = 0
    for col, col_type in trade_columns.items():
        if col not in existing_trades:
            try:
                cursor.execute(f"ALTER TABLE trades ADD COLUMN {col} {col_type}")
                print(f"  [OK] trades.{col} eklendi")
                added += 1
            except Exception as e:
                print(f"  [WARN] trades.{col} eklenemedi: {e}")

    # coin_profiles tablosu — yoksa oluştur
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS coin_profiles (
            symbol      TEXT PRIMARY KEY,
            win_rate    REAL DEFAULT 0.5,
            avg_rr      REAL DEFAULT 1.5,
            trade_count INTEGER DEFAULT 0,
            short_bias  REAL DEFAULT 0.5,
            updated_at  TEXT
        )
    """)
    print("  [OK] coin_profiles tablosu hazır")
    added += 1

    cursor.execute("PRAGMA table_info(coin_profiles)")
    existing_profiles = {row[1] for row in cursor.fetchall()}

    for col, col_type in coin_profile_columns.items():
        if col not in existing_profiles:
            try:
                cursor.execute(f"ALTER TABLE coin_profiles ADD COLUMN {col} {col_type}")
                print(f"  [OK] coin_profiles.{col} eklendi")
                added += 1
            except Exception as e:
                print(f"  [WARN] coin_profiles.{col} eklenemedi: {e}")

    conn.commit()
    conn.close()

    # _TRADE_COLUMNS cache'ini sıfırla
    try:
        import database as db_module
        db_module._TRADE_COLUMNS = None
        print("  [OK] _TRADE_COLUMNS cache sıfırlandı")
    except Exception as e:
        print(f"  [WARN] Cache sıfırlanamadı: {e}")

    if added == 0:
        print("[Migration] Tüm kolonlar zaten mevcut.")
    else:
        print(f"[Migration] {added} kolon eklendi.")

    print("[Migration] Tamamlandi.")


if __name__ == "__main__":
    migrate()
