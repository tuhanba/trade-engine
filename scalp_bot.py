...
def _sigterm_handler(signum, frame):
    logger.info("SIGTERM alındı, güvenli kapanış başlatılıyor...")
...
def main():
    ...
    # ── Real-time Trade Monitor (Hızlı PnL ve SL/TP kontrol) ──
    _monitor_stop = threading.Event()

    def _monitor_worker(binance_client, stop_event):
        logger.info("[Monitor] Worker thread başladı")
        while not stop_event.is_set():
            try:
                if EXECUTION_AVAILABLE:
                    from database import get_open_trades
                    if get_open_trades():
                        exec_monitor(binance_client)
            except Exception as _me:
                logger.warning(f"[Monitor] Worker hata: {_me}")
            stop_event.wait(timeout=2)   # 2 saniyede bir çalış
        logger.info("[Monitor] Worker thread durdu")

    _monitor_thread = threading.Thread(
        target=_monitor_worker,
        args=(client, _monitor_stop),
        daemon=True,
        name="monitor-worker",
    )
    ...
    while True:
        try:
            ...
            # Açık trade takibi (Artık background thread yapıyor)
            # if EXECUTION_AVAILABLE:
            ...
            for coin_info in eligible:
                ...
                try:
                    ...
                    will_execute_likely = (
                        AX_MODE == "execute" and EXECUTION_AVAILABLE
                        and sig.final_score >= _trade_thr
                        and sig.setup_quality in EXECUTABLE_QUALITIES
                    )

                    if will_execute_likely:
                        sig.telegram_status = "skipped_for_execution"
                    elif sig.final_score >= TELEGRAM_THRESHOLD:
